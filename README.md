...
> **MindTouch fork.** This is MindTouch's fork of
> [`WebSpellChecker/wproofreader-docker`](https://github.com/WebSpellChecker/wproofreader-docker).
> It tracks upstream and adds MindTouch-specific security hardening on top of the
> vendor Dockerfiles — currently `server_tokens off;` in `Dockerfile.ubuntu-prebuilt`
> to suppress NGINX version disclosure. Images built here are pushed to the
> MindTouch ECR repo `webspellchecker/wproofreader` and consumed by the
> `wproofreader` Helm chart in `MindTouch/Deki`. See internal issue
> `MindTouch/Deki#39008` for background. Keep changes minimal and rebase on
